...
class ConsultaProcesoController {
  ...
  verificadorEstado = async () => {
    if (this.#rol === 1 || this.#rol === 2 || this.#rol === 4) {
      if (this.#defensor.value !== '0') {
        const { procesosJudiciales } = await this.model.getProcesosBusqueda(this.#defensor.value, null, false, this.#pagina, "EN_TRAMITE")
        this.#procesos = procesosJudiciales
        this.getNumeroPaginas(this.#defensor.value)
        this.fillTabla()
      }
      else {
        const { procesosJudiciales } = await this.model.getProcesosBusqueda(null, this.model.user.id_distrito_judicial, false, this.#pagina, "EN_TRAMITE")
        this.#procesos = procesosJudiciales
      ...
      const { procesosJudiciales } = await this.model.getProcesosBusqueda(this.#idEmpleado, null, false, this.#pagina, "EN_TRAMITE")
      this.#procesos = procesosJudiciales
      this.fillTabla()
    }
  }
  ...
  getNumeroPaginas = async (id_defensor) => {
    try {
      ...
      const { totalProcesosJudiciales } = await this.model.getProcesosBusqueda(id_defensor || null, id_defensor === null ? id_distrito_judicial : null, true, 1, "EN_TRAMITE")
